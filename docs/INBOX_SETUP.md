...
> สถานะ: โค้ดเสร็จ + tsc 0 error · **ยังไม่เปิดใช้งานจริง** จนกว่าจะทำขั้นด้านล่าง

---

## 🆕 อัปเดตหลัง /auditbigteam (2026-05-29) — ต้องทำเพิ่ม
แก้บั๊ก 7 P0 แล้ว (webhook async, กันส่งซ้ำ, ลูกค้าโกรธไม่ตอบผิด, รับรูป, เด้งเตือนเคสด่วน, ฟิลเตอร์จริง). สิ่งที่ต้องตั้งเพิ่ม:
1. **AI = Gemini** (ไม่ต้องใช้ Anthropic) → ใช้ `GEMINI_API_KEY` ที่มีใน prod อยู่แล้ว ✅ ไม่ต้องทำอะไร
2. **migration ใหม่** (กันข้อความซ้ำ): รัน `supabase/migrations/20260529000000_inbox_message_idempotency.sql` ใน Supabase SQL Editor (เพิ่มจากตัวแรก)
3. **env ใหม่ `INBOX_ALERT_TARGET`** (เด้งเตือนเคสด่วน): ตั้งใน Vercel = LINE userId หรือ groupId ของทีม/CEO ที่จะรับแจ้งเตือน (เช่น userId ของ CEO: `Uf86a231ecc017919604ecb63cd0e80a8`) → เคสด่วน (เงินหาย/สแกนไม่ได้/ลูกค้าส่งรูป) จะ push เข้าไลน์นั้นทันที
