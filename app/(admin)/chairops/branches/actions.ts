...
// Close / reopen a branch (CEO 2026-08-23) — for locations that are no
// longer operating (e.g. mall zone shut down) so they stop cluttering the
// active roster/dashboard/reconcile screens, while still being visible
// (muted, sunk to the bottom) on the maid activity table rather than just
// vanishing. ADMIN+ only — this is repo-wide-visible, bigger than the
// per-page mutations above.
const branchIdInput = z.object({ branchId: z.string().uuid() });

export async function closeBranch(branchId: string): Promise<ActionResult<{ closedAt: string }>> {
  const session = await requireRole("ADMIN");
  const parsed = branchIdInput.safeParse({ branchId });
  ...
  if (!branch) return { ok: false, error: "ไม่พบสาขา" };
  if (!branch.isActive) return { ok: false, error: "ปิดสาขาไปแล้ว" };

  // Settle gate (same principle as deactivateUser's F6 check) — don't let a
  // branch closure quietly hide cash that was collected but never deposited.
  const pendingCount = await prisma.chairopsCashCollection.count({
    where: { orgId: session.user.orgId, branchId: branch.id, depositId: null, deletedAt: null },
  });
  if (pendingCount > 0) {
    return { ok: false, error: `มีรายการเก็บเงินที่ยังไม่ฝาก ${pendingCount} รายการ — ต้องฝากให้ครบก่อนปิดสาขา` };
  }
  ...
  await writeAudit({
    ...
    newValue: { isActive: false, closedAt: now.toISOString() },
    metadata: { route: "/chairops/maids" },
  });

  revalidatePath("/chairops/maids");
  revalidatePath("/chairops/branches");
  return { ok: true, data: { closedAt: now.toISOString() } };
}

export async function reopenBranch(branchId: string): Promise<ActionResult<void>> {
  const session = await requireRole("ADMIN");
  const parsed = branchIdInput.safeParse({ branchId });
  if (!parsed.success) return { ok: false, error: "ข้อมูลไม่ถูกต้อง" };
  ...
  if (!branch) return { ok: false, error: "ไม่พบสาขา" };
  if (branch.isActive) return { ok: false, error: "สาขาเปิดอยู่แล้ว" };
  ...
  await writeAudit({
    userId: session.user.id,
    action: "branch.reopen",
    entity: "ChairopsBranch",
    entityId: branch.id,
    oldValue: { isActive: false },
    newValue: { isActive: true, closedAt: null },
    metadata: { route: "/chairops/maids" },
  });
