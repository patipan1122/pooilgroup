...
import { startGmailConnect, disconnectGmail } from "./actions";
    ...
    <div>
      <button
        ...
        onClick={() =>
          start(async () => {
            setErr(null);
            const res = await startGmailConnect();
...
export function GmailDisconnectButton() {
  ...
  return (
    <button
      ...
      disabled={pending}
      onClick={() => {
        if (!confirm("ยกเลิกการเชื่อม Gmail? (ข้อมูลที่ import แล้วไม่ถูกลบ)")) return;
        start(async () => {
