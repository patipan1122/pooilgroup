 ...
 *
 * Ported verbatim from `~/ตู้คีบ/src/page-hub.jsx` (the HubPage component +
 * its sub-components) to a Next.js 15 App Router client page.
 *
 * Layout chrome (Sidebar + TopBar + cf-app/cf-main/cf-content) is rendered by
 ...
 *   - `branch`        → read from the `?branch=` searchParam (default "all")
 ...
 *
 * TODO[v2-wire-db]: the data still comes from `lib/clawfleet/v2-data` (mock).
 */
...
export default function HubPage() {
  ...
  const anomalies = useMemo(
    () => ANOMALIES.filter((a) => inScope(branch, a.branchId)),
    [branch],
  );
  const sessions = useMemo(
    () => ACTIVE_SESSIONS.filter((s) => inScope(branch, s.branchId)),
    [branch],
  );
  ...
  // note arg is accepted by the AnomalyReview contract but unused on the Hub
  const decide = (decision: string) => {
    ...
    if (!reviewing) return;
    const i = anomaliesSorted.findIndex((x) => x.id === reviewing.id);
    const remaining = anomaliesSorted.filter((x) => x.id !== reviewing.id);
      ...
      <div className="cf-page">
        ...
        <div className={`cf-hero is-${heroVariant}`}>
          <div className="cf-hero-revenue">
            <div className="cf-hero-label">รายได้วันนี้ · {TODAY.sessions} รอบจาก 10 สาขา</div>
            <div className="cf-hero-amount">
              <span className="cf-hero-currency">฿</span>
              <span className="cf-hero-num">{TODAY.revenue.toLocaleString("th-TH")}</span>
            </div>
            <div className="cf-hero-meta">
              ...
              <span className="cf-dim">
                {TODAY.prizesOut} ตุ๊กตาคีบไป · {TODAY.staffActive}/{TODAY.staffTotal} พนง.ออนไลน์
              </span>
            </div>
            {showSparkline && <Sparkline data={TREND_7D} />}
          </div>
          ...
          >
            <div className="cf-stock-list">
              {stockLow.slice(0, 5).map((s, i) => (
                <StockRow key={s.branchId + s.sku + i} s={s} />
              ))}
            </div>
          </Section>

          <Section
            title={`รอบที่ปิดแล้ววันนี้ · ${CLOSED_TODAY.length}`}
...
function StockRow({ s }: { s: StockLow }) {
