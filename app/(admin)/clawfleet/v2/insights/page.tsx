 ...
 *
 * Ported from `InsightsPage` in `~/ตู้คีบ/src/page-rest.jsx`.
 * Renders ONLY the `.cf-page` body — Sidebar + TopBar come from the V2Shell
 * client layout (`components/clawfleet/v2/shell.tsx`).
 *
 * Branch filter is read from the `?branch=` searchParam. Tab pills + range
 * buttons are local UI state. The dense `cf-table` lists INSIGHTS_ROWS filtered
 * by branch with cross-check gap + status pills.
 */

import { useSearchParams } from "next/navigation";
import { useState } from "react";
import { Ic, Pill, StatTile, fmtTHB } from "@/components/clawfleet/v2/chrome";
import { INSIGHTS_ROWS, getBranch } from "@/lib/clawfleet/v2-data";
...
export default function InsightsPage() {
  ...
  const rows = INSIGHTS_ROWS.filter((r) => branch === "all" || r.branchId === branch);
