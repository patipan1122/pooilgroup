 ...
 *
 * Ported verbatim from the `StockPage` component in `~/ตู้คีบ/src/page-rest.jsx`
 * (lines ~224-354) to a Next.js 15 App Router client page.
 *
 ...
 *   - `branch` → read from the `?branch=` searchParam (default "all"); used to
 *     seed the local `activeBranch` chip state.
 *
...
import { BRANCHES, BRANCH_STOCK, DELIVERIES, getBranch } from "@/lib/clawfleet/v2-data";

export default function StockPage() {
  const branch = useSearchParams().get("branch") ?? "all";

  const [activeBranch, setActiveBranch] = useState<string>(
    branch === "all" ? BRANCHES[0].id : branch,
  );

  const stock = BRANCH_STOCK[activeBranch] ?? [];
  ...
  const branchDeliveries = DELIVERIES.filter((d) => d.branchId === activeBranch);
