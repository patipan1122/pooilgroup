...
import type { HotelChannelConfig } from "@/lib/cashhub/hotel-settlement-data";
...
export function HotelSettingsEditor({ configs, accounts, companies, branchCode = "" }: Props) {
  ...
  const patch = (channel: string, p: Partial<HotelChannelConfig>) =>
  ...
  return (
    <div className="space-y-4">
            ...
            return (
              <li key={r.channel} className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm">
                ...
                {!r.isSettle ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-0.5 text-emerald-700">
                    {acctLabel(acc)}
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-2.5 py-0.5 text-amber-700">
              ...
              <tr key={r.channel} className="border-b border-zinc-100">
                ...
                <td className="p-3 text-center">
                  <input
                    type="checkbox"
                    checked={r.isSettle}
                    onChange={(e) => patch(r.channel, { isSettle: e.target.checked })}
                ...
                <td className="p-3 text-right">
                  <input
                    ...
                    value={r.feePercent}
                    disabled={!r.isSettle}
                    onChange={(e) => patch(r.channel, { feePercent: Number(e.target.value) })}
                ...
                <td className="p-3">
                  <select
                    ...
                    disabled={!r.isSettle}
                    onChange={(e) => patch(r.channel, { bankAccountId: e.target.value || null })}
                ...
                <td className="p-3">
                  <select
                    value={r.companyId ?? ""}
                    disabled={!r.isSettle}
                    onChange={(e) => patch(r.channel, { companyId: e.target.value || null })}
      ...
      <p className="text-xs text-zinc-500">
        ทุกช่องที่ติ๊ก &ldquo;เป็นเงินเข้าธนาคาร&rdquo; + เลือกบัญชี จะถูกส่งเข้าหน้ากระทบยอดของบัญชีนั้น
        (บัญชีใครบัญชีมัน) · ช่องที่ไม่ติ๊ก จะไม่ส่ง · เงินเข้าจริง = ยอดช่องทาง − ค่าธรรมเนียม
      </p>
    </div>
  );
}
