// Playland · รับเด็กเข้าเล่น (Check-in workspace · /playland/checkin · ย้ายมาจาก cockpit เดิม)
//
...
export default async function PlaylandCheckin({ searchParams }: { searchParams: Promise<{ branch?: string; selected?: string }> }) {
    ...
    <div className="pl-page">
      ...
      <header className="pl-header">
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <Link href={`/playland?branch=${branchId}`} className="pl-btn pl-btn-ghost">← หน้าหลัก</Link>
          ...
          {activeSessions.length === 0 ? (
            ...
            <ul className="pl-stagger" style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {activeSessions.map((s) => {
                ...
                const href = isSel ? `/playland/checkin?branch=${branchId}` : `/playland/checkin?branch=${branchId}&selected=${s.id}`;
                return (
                  <li key={s.id}>
        ...
        <aside className="pl-pane">
          {selectedSession ? (
            <SessionInspector
              ...
              backHref={`/playland/checkin?branch=${branchId}`}
            />
          ) : (
            <>
              <div className="pl-pane-head">
