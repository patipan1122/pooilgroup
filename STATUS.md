...
**ข่าวร้าย/สำคัญที่สุด:** เลข "เงินขาด" หลักที่ Dashboard/แจ้งเตือน/อนุมัติตัดหนี้สูญใช้ ยังไม่ถูกแก้จริง — มีการสร้างหน้า "รอบเก็บ" ใหม่ที่คำนวณถูกวิธี (เทียบยอดแม่บ้านนับได้ ไม่ใช่แค่ยอดฝาก) แต่สูตรถูกใช้แค่หน้าใหม่หน้าเดียว ส่วนเลขที่ระบบอื่นอ่านยังเป็นสูตรเก่า — 2 สูตรมีเครื่องหมาย +/- กลับด้านกัน ไม่มีระบบเช็ค sync อัตโนมัติ (Δ4 ในเอกสาร)
...
**ต่อไป:** `/bigsolvebug ChairOps` ตามคิวที่ CEO สั่ง — จะแก้ fire-and-forget + เชื่อม driftAmount ให้ใช้สูตรที่ถูกเป็นแหล่งเดียว (งานเงิน ต้องผ่าน FIN+OFC+AUD lens ก่อน merge) แล้วต่อด้วย `/upspeed`
