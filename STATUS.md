...
> **Source of truth สำหรับสถานะจริง** — อัพเดต 2026-09-09 (🏬🚀 RentSpace matrix — ปุ่ม "ส่งเข้าบัญชี LedgerLine" ย้ายมาไว้หน้าตารางค่าเช่า + ด่านเช็คยอดสลิปก่อนส่ง (เจอ 4 บิลจริงยอดไม่ตรง กันไว้ไม่ให้ส่ง) — BUILT+verified, กำลัง push · 🧾🔧🚀 LedgerLine VAT อ่านผิดเป็น 0 บนบิลราคาต่อชิ้นรวม VAT — **DEPLOYED** `7de3ec1e`, backfill ใบ Dohome แล้ว, ไล่เช็ค 8 ใบทั้งระบบพบอีก 1 ใบโดนเหมือนกัน (AP 551563 — ผูกกับปัญหา GL/PV เดิมที่ค้างอยู่) รอ CEO ตัดสินใจ · 🪑📤✅ ChairOps เลือกหลายสาขาส่งเข้า reconcile ทีเดียว (จาก Pinpoint) — **DEPLOYED LIVE** `4aa39276` · 🧾⚡ LedgerLine รายจ่าย คลิกเปลี่ยนบิลรู้สึกเหมือน refresh — **DEPLOYED LIVE** `6d2b33c3`, smoke ยืนยันแล้ว · 🏬🧾 RentSpace export รายงานสรุปค่าเช่าจากหน้า matrix — **DEPLOYED LIVE** `61fb3638`, smoke ยืนยันแล้ว · 🦞🧾 ClawFleet แนบสลิปฝากเงิน+AI อ่านยอด จากหน้าประวัติเก็บเงิน — **DEPLOYED LIVE** `80b37319` · 🧾 RentSpace คลิกดูสลิป+AI ตรวจสลิปต่อรายการชำระ — **DEPLOYED LIVE** `c3ac784f`, รอ CEO ตั้งค่าบัญชีธนาคารก่อนใช้จริง · ✅ ChairOps "ควรได้"(มิเตอร์) บั๊ก zero-fallback org-wide — DEPLOYED LIVE `edbe8832`)

## 🏬🚀 RentSpace matrix — ปุ่ม "ส่งเข้าบัญชี LedgerLine" บนหน้าตารางค่าเช่า + ด่านเช็คยอดสลิปก่อนส่ง (2026-09-09 · BUILT+verified, กำลัง push)
...
**ยังไม่ push — กำลัง verify+deploy รอบนี้**
...
## 🧋💸✅ CashHub Tea — เจอเงินหาย ฿31,268 เพราะช่องทางชำระไม่มีถังรองรับ + K Plus ปนผิดบัญชี (2026-08-23 · DEPLOYED `53b0bae8`)

CEO เปิดไฟล์ Foodstory ดิบใน Numbers เอง เจอว่าคอลัมน์ "ประเภทการชำระเงิน" มีค่า "K Plus" กับ "ไทยช่วยไทยพลัส" ปนอยู่ — บอกตรงๆ ว่า "เราพลาดเอง" อยากให้แยกเป็นช่องทางของตัวเอง ส่งเข้าคนละบัญชีได้ + ดูไส้ในได้เหมือน QR

**เช็ค DB ก่อนแก้ (พบเรื่องใหญ่กว่าที่คิด):**
- **"ไทยช่วยไทยพลัส" ไม่มีกฎรองรับเลยตั้งแต่แรก** → ตกไปถัง "อื่นๆ" ซึ่ง**ไม่ส่งเข้า reconcile** — เท่ากับเงินหายจากระบบบัญชีเงียบๆ **฿31,268 สะสม 74 วัน** (ทุกสาขารวมกัน) ไม่ใช่แค่ตัวเลขเล็กๆ
- **"K Plus" ถูกจัดปนกับ QR ทั่วไป** — ถ้าจริงๆ เงิน K Plus เข้าคนละบัญชีกับ QR ทั่วไป ยอดที่ผ่านมาจะส่งไปกระทบยอดผิดบัญชี

**FIX** ([`tea-channels.ts`](lib/cashhub/tea-channels.ts)): เพิ่ม 2 ช่องทางใหม่ `kplus` และ `thaichuaithaiplus` แยกจาก QR/other ชัดเจน — เพราะทุกจุดในระบบ (คอลัมน์ตาราง, หน้าตั้งค่าบัญชีปลายทาง, ส่งเข้า reconcile แบบรายรายการ, หน้าไส้ใน) อ่านรายชื่อช่องทางจาก `TEA_CHANNELS` แบบอัตโนมัติอยู่แล้ว **ไม่ต้องแก้ที่อื่นเลย** พอเพิ่มช่องทางใหม่ก็ใช้งานได้ครบทุกฟีเจอร์ทันที

**Verify:** tsc 0 error · eslint 0 error · `next build` ผ่าน · smoke test `/` `/cashhub/tea` `/cashhub/tea/settings` → 200 ทุกตัว

📝 **ยังไม่ย้อนแก้ข้อมูลเก่า** — ยอด ฿31,268 ที่เคยตกหล่น + ยอด K Plus ที่ปนกับ QR ต้อง**อัปโหลดไฟล์เดือนที่เกี่ยวข้องซ้ำ**ถึงจะจัดเข้าถังใหม่ที่ถูกต้อง (โค้ดเปลี่ยนไม่ได้ทำให้ข้อมูลเก่าขยับเอง) · หลังอัปโหลดซ้ำ CEO ต้องไปตั้งบัญชีปลายทางของ K Plus + ไทยช่วยไทยพลัส ที่หน้าตั้งค่า CashHub Tea ก่อน (ช่องใหม่ยังไม่ผูกบัญชี)
...
**🚨 พบผลกระทบจริงจากบั๊กนี้:** เช็ค production DB (read-only) พบสาขา **"Centralอยุธยา" หายไปจากหน้าตรวจยอดตั้งแต่เช้านี้ (2026-08-23 05:20)** เพราะโดนปุ่มเก่าของผมตั้ง `isActive=false` ไป (อธิบายว่าทำไม CEO เห็นหน้าตรวจยอดขึ้น "47 สาขา" แทนที่จะเป็น 48) — **ยังไม่ได้แก้ข้อมูล รอ CEO อนุมัติก่อน** (ดูรายละเอียดในแชท)
- เช็คแล้วพบอีก 1 สาขา ("robinsonปราจีน") ที่ isActive=false เหมือนกัน แต่ปิดมาตั้งแต่ 2026-07-08 (ก่อนงานนี้เริ่มเป็นเดือน) — ไม่ใช่บั๊กจากงานนี้ ไม่แตะ

**FIX:** ลบปุ่ม/action "ปิดสาขา" ที่ผมสร้างเองทิ้งทั้งหมด (`closeBranch`/`reopenBranch` ใน `chairops/branches/actions.ts`) — เปลี่ยนปุ่มที่หน้าแม่บ้านให้เรียก `toggleBranchClosedAction` ตัวเดียวกับหน้าตรวจยอดแทน (ไม่มี confirm dialog ด้วย ตามแบบที่ใช้อยู่แล้ว) + แก้ query หน้าแม่บ้านให้เช็คสถานะ "ปิด" จาก `closedAt` (ไม่ใช่ `isActive`) ให้ตรงกับหน้าตรวจยอด — ตอนนี้กดปิด/เปิดสาขาจากหน้าไหนก็ได้ผลเหมือนกันทุกหน้าจริงๆ

**ไฟล์ที่แตะ:** [`branch-close-buttons.tsx`](app/(admin)/chairops/(office)/maids/_components/branch-close-buttons.tsx) (เรียก action เดิม), [`chairops/branches/actions.ts`](app/(admin)/chairops/branches/actions.ts) (ลบ action ซ้ำทิ้ง), [`maid-roster.ts`](lib/chairops/queries/maid-roster.ts) (เช็ค closedAt แทน isActive), [`reconcile/actions.ts`](lib/chairops/reconcile/actions.ts) (เพิ่ม revalidate หน้าแม่บ้านด้วย)
...
## 🧋🔑✅ CashHub Tea QR itemized — จับคู่อัตโนมัติ 0/887 รายการ — ไม่ใช่บั๊กจากรอบ itemize (2026-08-23)

CEO ส่งยอด QR แบบรายรายการรอบใหม่แล้วกด "จับคู่อัตโนมัติ" — ระบบบอก "ไม่พบรายการที่จับคู่ได้" ทั้งที่ตา/บาทตรงกันชัดเจนหลายคู่ (เช่น ฿68 ทั้งสองฝั่ง วันเดียวกัน)

**สาเหตุจริง (เช็ค DB ตรงก่อนสรุป):** บัญชีธนาคาร ttb ****4274 (ใช้ QR ของร้านชาไข่มุกล้วนๆ 8 สาขา ไม่มีช่องทางอื่นปนเลย — ยืนยันจาก DB) ระบบข้อความในสเตทเมนต์จริงเป็น **"Transfer in by CitizenID via KTB/SCB/..."** ไม่มีคำว่า "QR"/"พร้อมเพย์" เลยสักคำ ขณะที่กฎจับคู่ QR (`MATCH_CONCEPTS.qr` ในโค้ด) บังคับให้ต้องเจอคำว่า "qr"/"promptpay"/"พร้อมเพย์" ในข้อความธนาคารก่อนถึงจะลองจับคู่ — เจอคำเลยไม่จับเลยสักคู่ **ปัญหานี้มีอยู่ก่อนหน้านี้แล้ว** (ยอดก้อนรวมเก่าก็ไม่เคยจับคู่ได้ด้วยเหตุผลเดียวกัน — เช็คแล้วว่าไม่ใช่เพราะเปลี่ยนเป็นรายรายการ)

**FIX (ไม่ใช่แก้โค้ด — ใช้ฟีเจอร์ "สมุดจำคีย์" ที่มีอยู่แล้ว):** เพิ่มคีย์เวิร์ด `"transfer in"` เข้าสมุดจำคีย์ (`ledger_bank_match_keyword`) **เฉพาะบัญชีนี้บัญชีเดียว** ไม่กระทบบัญชี/โมดูลอื่น — ปลอดภัยเพราะบัญชีนี้รับแต่เงิน QR ร้านชาเท่านั้น (ยืนยันจาก DB ก่อนเพิ่ม) เงินโอนเข้าบัญชีนี้ทุกรายการจึงถือเป็น QR ได้เลย

📝 CEO กด **"จับคู่อัตโนมัติ"** อีกครั้งที่หน้า LedgerLine บัญชี ttb ****4274 — ควรเริ่มจับคู่ได้แล้ว
...
**FIX:** กลับไปใช้ตาราง 7 คอลัมน์เดิม (สาขา/ชื่อ/ทำงานมาแล้ว/เก็บเงินล่าสุด/เฉลี่ยฝากเงิน/บัญชี/เวลาที่มักเก็บ/ฝาก) แต่ยังคง 1 แถว/สาขา (กันบั๊กชื่อสาขาซ้ำที่แก้ไปแล้ว) — คอลัมน์สถิติทั้งหมดโชว์ข้อมูลของ "คนล่าสุดที่ทำงาน" (เทียบจาก `lastCollectedAt` ล่าสุดในสาขานั้น) ถ้าสาขามีมากกว่า 1 คน จะมีป้าย "N คน" กดเปิด popup ดูทุกคน + ปุ่มลาออกได้เหมือนเดิม · ปุ่ม "ปิดสาขา" ย้ายมาอยู่ติดหลังชื่อสาขาตามที่ขอ (ไม่ใช่คอลัมน์แยกขวาสุดอีกแล้ว)
...
## 🦞➖✅ ClawFleet ผูกบัญชี — เอาพนักงานออกจากสาขาได้จาก popup เดียวกัน (2026-08-23 · 🚀 DEPLOYED `f172855b`)

ต่อจาก [[clawfleet-reconcile-prep-decisions-2026-08-23]] (popup ดู/เพิ่มพนักงาน deployed `2cae07bd`) — CEO: "ควรจะเตะออกได้ด้วย ... อย่าลืมว่าข้อมูลต้องเชื่อมต่อกันทั้งโปรแกรม"

**สิ่งที่ทำ:** เพิ่มปุ่ม "เอาออก" ต่อรายชื่อพนักงานใน popup — **reuse `removeCfStaff()` ตัวเดียวกับหน้า "พนักงาน" เป๊ะ** (ข้อความยืนยัน/พฤติกรรมปิดบัญชีอัตโนมัติถ้าไม่เหลือสาขา/audit log — เหมือนกันทุกจุด ไม่ได้เขียน logic ลบซ้ำ) ตามที่ CEO ย้ำเรื่องข้อมูลต้องเชื่อมกันทั้งโปรแกรม · บทบาทระดับแอดมินองค์กร (org_admin/super_admin/admin/program_admin) ยังถูกกันไว้เหมือนหน้าเดิม — โชว์ "ผู้ดูแลองค์กร" แทนปุ่มเอาออก

**Verify:** tsc/eslint/`next build` สะอาดทั้งหมด · rebase ชน commit คู่ขนาน ChairOps แม่บ้านอีกรอบ (คนละไฟล์ ไม่ conflict) · push `origin/setup` (`f172855b`) → vercel inspect + smoke test ผ่านเหมือนเดิม
...
## 🦞👥✅ ClawFleet ผูกบัญชี — ดูรายชื่อ + เพิ่มพนักงานจากตารางเดียวกันได้เลย (2026-08-23 · 🚀 DEPLOYED `2cae07bd`)

ต่อจาก [[clawfleet-reconcile-prep-decisions-2026-08-23]] (ตาราง "สาขานำหน้า" deployed `645ebef0`) — CEO ส่ง screenshot ของตารางที่เพิ่งขึ้น บอกว่าอยากให้กด **ดูชื่อพนักงาน** และ **เพิ่มพนักงาน** ผ่าน popup ได้จากหน้าเดียวกันเลย ไม่ต้องสลับไปหน้าพนักงานแยก

**สิ่งที่ทำ:**
- คอลัมน์ "พนักงาน" (เดิมโชว์แค่ตัวเลข) → กดได้ เปิด popup แสดงรายชื่อ+ตำแหน่งของสาขานั้น + ปุ่ม "+ เพิ่มพนักงาน" ในหน้าต่างเดียวกัน — กรอกชื่อ/บทบาท/อีเมล-เบอร์(ไม่บังคับ) → ได้ลิงก์เชิญให้คัดลอกส่งพนักงาน (ระบบเดิมเป๊ะ ใช้ `inviteCfStaff()` ตัวเดียวกับหน้า "พนักงาน")
- **จงใจไม่ทำ** แก้บทบาท/ปิดใช้งานซ้ำที่นี่ — คงไว้ที่หน้าพนักงานเดิม กัน popup นี้บวมเกินจำเป็น (Rule L)
- [`lib/clawfleet/reconcile/overview.ts`](lib/clawfleet/reconcile/overview.ts) เปลี่ยนจาก query นับจำนวนเอง → ดึงรายชื่อพนักงานจาก `getTeamData()` ตัวเดียวกับหน้าพนักงาน (single source — "มีพนักงานไหม" ไม่มีทางไม่ตรงกันระหว่าง 2 หน้า)

**Verify:** tsc/eslint/`next build` สะอาดทั้งหมด · rebase ชน commit คู่ขนานอีก 2 ครั้ง (ย้ายตู้ข้ามสาขา + จัดเรียงเช็คลิสต์) ไม่มี conflict เพราะคนละไฟล์ · push `origin/setup` (`2cae07bd`) → vercel inspect ยืนยัน alias ใหม่ + smoke test `/` `/login` `/clawfleet/os/branches` `/clawfleet/os/staff` → 307/200/307/307 เหมือนเดิม
...
**Round 2 (2026-08-23, same day) — CEO กดตัวเลขในตารางแล้วไม่เห็นไส้ในเลย + ขอเพิ่มปุ่มสลับเก็บ/ฝาก.**
สาเหตุจริง (ตรวจโค้ดแล้ว ไม่ใช่เดา): `DayDetailPanel` เดิมเรนเดอร์เป็น block ธรรมดาแปะอยู่**เหนือ**ตาราง — เช็คลิสต์ตัวเลขมี 48 แถวสาขา ถ้า CEO เลื่อนจอลงมาก่อนกดตัวเลข การ์ดไส้ในจะโผล่เหนือจุดที่มองเห็นอยู่ (ลิงก์ใช้ `scroll={false}` เพื่อไม่ให้กระโดดกลับบนสุด) → ดูเหมือน "กดแล้วไม่มีอะไรเกิดขึ้น" ทั้งที่จริงมันโผล่แล้วแค่อยู่นอกจอ. เจอ bug เดียวกันแฝงอยู่ในแท็บ Ledger ด้วย (โครงสร้างเดียวกัน) แก้จุดเดียวใน `DayDetailPanel` ที่ใช้ร่วมกันทั้งสองที่.
- **แก้:** ห่อ `DayDetailPanel` เป็น popup ลอย (`position:fixed`, กึ่งกลางจอ, มี backdrop กดปิดได้) แทนที่จะเป็น block ธรรมดา — ยังไม่มี client JS เหมือนเดิม (backdrop เป็น `<Link>` ธรรมดา ไม่ใช่ onClick)
- **เพิ่ม:** ปุ่มสลับ "🧺 ยอดเก็บ / 🏦 ยอดฝาก" ในป็อปอัพ (checkbox+`:has()` แบบเดียวกับปุ่มจำนวนเงิน/ผลต่าง) — โชว์เฉพาะตอนวันนั้นมีทั้งเก็บและฝาก (ถ้ามีแค่ฝั่งเดียวไม่โชว์ปุ่ม เห็นฝั่งที่มีตามปกติ)
- ไม่แตะ query/data-layer เลยรอบนี้ (`reconcile-v2.ts` เดิมทั้งไฟล์) — แก้แค่ `reconcile-views.tsx` (JSX) + `reconcile-v2.css`
- **⚠️ verify รอบนี้เบากว่าปกติ — ต้องบอกตรงๆ:** เครื่อง dev ตอนนั้นโหลดหนักมาก (load average พุ่งถึง 163 จากหลาย session ทำงานพร้อมกัน) รัน `tsc --noEmit` เต็มโปรเจกต์ 2 รอบไม่จบ (รอรวม ~1.5 ชม. แล้ว kill ทิ้งทั้งคู่) — CEO อนุมัติให้ไปต่อด้วยการตรวจที่เบากว่า: eslint ไฟล์ที่แก้ผ่านสะอาด (0 error) + อ่าน diff ทวนมือทั้งหมด (โครงสร้าง JSX/CSS สมดุล ไม่มีชนิดข้อมูล/ signature ไหนเปลี่ยน) แทน. ตัวตรวจที่แท้จริงที่ยังผ่านอยู่คือ **Vercel build จริงตอน deploy** (คนละเครื่อง ไม่โดน contention ในนี้) — เช็คว่า deployment ขึ้น Ready ก่อนถือว่าผ่าน. ควรรัน `tsc --noEmit` เต็มโปรเจกต์ซ้ำอีกทีตอนเครื่องว่างเพื่อความชัวร์เต็มร้อย (ยังไม่ได้ทำ).
