...
## 🧑‍💼🔗🚧 Recruit — ลิงก์แชร์ดูผู้สมัคร ต่อประกาศเดียว (2026-09-19 · BUILT+VERIFIED ใน worktree, ยังไม่ apply migration + ยังไม่ push)

CEO ถามหาลิงก์ส่งให้คนในทีม (ที่ไม่มีสิทธิ์ Recruit) ดูรายชื่อผู้สมัครของประกาศเดียวได้ — เช็คโค้ดแล้วไม่มีของเดิม (ปุ่ม "ดู N ใบ" เดิมต้องล็อกอินเป็นแอดมิน Recruit เท่านั้น, ไม่มี export CSV/PDF) เป็นฟีเจอร์ใหม่ CEO เลือก 2 เงื่อนไข: (1) ต้องล็อกอินก่อนถึงดูได้ (ไม่ public เต็มที่) (2) ขอบเขตต่อประกาศเดียว ไม่ใช่ทั้งโปรแกรม

**สร้างแล้ว (worktree `pg-wt-recruit-share`, branch `claude/recruit-applicant-share`, ยังไม่ push):**
- คอลัมน์ token ใหม่ต่อประกาศ (`applicant_share_token`, unique, สุ่มไม่ซ้ำ) — migration `20260919120000_recruit_applicant_share_token.sql` (สร้างไฟล์แล้ว **ยังไม่ apply เข้า DB จริง**)
- ปุ่ม "แชร์ดูผู้สมัคร" หน้ารายการประกาศ — สร้าง/คัดลอก/ยกเลิกลิงก์ได้
- หน้าดูอย่างเดียวที่ `/recruit-share/[token]` — ต้องล็อกอิน (บัญชี Pooilgroup อะไรก็ได้ ไม่ต้องมีสิทธิ์ Recruit) เห็นชื่อ-เบอร์-ไฟล์แนบ-สถานะ-วันที่สมัคร ต่อประกาศนั้น **ไม่โชว์คะแนน/สรุป AI ภายใน** (ข้อมูลประเมินของแอดมินสงวนไว้)

**Verify:** tsc 0 errors (whole project), eslint 0 errors บน 5 ไฟล์ที่แตะ, `next build` compiled สำเร็จ (125 หน้า รวม route ใหม่), `check-schema-applied` ยืนยันคอลัมน์ยังไม่มีจริงใน DB (ตามคาด เพราะยัง apply migration ไม่ได้)
