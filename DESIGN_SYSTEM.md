...
✗ ใช้สีจัดประเภท (เช่น 3 module = 3 สี)
  ...
  → ใช้ฟ้าเหมือนกันหมด แยกด้วย icon lucide ชุดเดียว (ห้าม emoji — ดู §11)
...
## 11. กฎกัน "หน้าตาเหมือน AI" (Anti-AI-Slop) — LOCKED 2026-06-02

> CEO ระบุชัด 2026-06-02: "ปุ่ม/ไอคอน/โลโก้/emoji ดูเหมือนฟรี AI ไม่มืออาชีพ"
> section นี้ **เหนือกว่า** guidance เดิมทุกข้อที่ขัดกัน (เช่น §2 ที่เคยบอกให้ใช้ emoji แยกโมดูล)

### 11.1 ห้าม emoji ใน UI จริง (เด็ดขาด)
```
✗ emoji ทุกชนิดใน production UI — ปุ่ม · หัวข้อ · การ์ด · nav · badge · empty state
✗ emoji แยกประเภทโมดูล (ยกเลิกกฎเดิม §2)
✗ emoji ตกแต่ง 🚀✨🎉🔥💡 — สัญญาณ "ทำโดย AI" อันดับ 1
✓ ใช้ icon จาก lucide-react แทนทุกที่ (โทนเส้นเดียวกันทั้งระบบ)
※ ข้อยกเว้น: emoji ใน "ข้อความที่ผู้ใช้พิมพ์เอง" (chat/inbox) ไม่นับ — ห้ามเฉพาะ chrome ของแอป
```

### 11.2 ไอคอน = ชุดเดียว lucide-react
```
✓ lucide-react เท่านั้น · size 16/20/24 · stroke-width เดียว (2) · สี currentColor หรือ brand-600
✗ ผสมหลายชุด (lucide + heroicons + fontawesome + emoji)
✗ ไอคอน gradient / 3D / โหลดมั่วจากเน็ต / สีรุ้ง
```

### 11.3 โลโก้ + รูปภาพ = ของจริง ไม่ใช่ AI
```
✗ โลโก้/ภาพที่ AI สร้างเอง · clip-art · stock ดูปลอม · hero ที่ดูเป็น AI-generated
✓ โลโก้แบรนด์จริงของลูกค้า (SVG) · ถ้าไม่มี → wordmark ตัวอักษร (Anuphan 800) + สัญลักษณ์เรขาคณิตเรียบ
✓ ถ้าต้อง illustration → เส้นเรียบโทนแบรนด์ (แบบ PhotoArt SVG ใน hotelbook) ไม่ใช่ภาพถ่าย AI
```

### 11.4 "สัญญาณ AI-slop" — checklist ก่อน ship
```
☐ ไม่มี emoji ใน chrome ของแอปเลย (ทั้งหน้า)
☐ ไอคอนชุดเดียว (lucide) สี/ขนาด/เส้น สม่ำเสมอ
☐ ไม่มี gradient ม่วง/ชมพู/teal (สัญญาณ template AI)
☐ ไม่ใช่จัดกลางทุกอย่าง — มี hierarchy ซ้าย-ขวาชัด
☐ ข้อมูลเป็นของจริง ไม่ใช่ placeholder "Lorem / 1,234 / John Doe"
☐ ปุ่มหลัก 1 ปุ่มต่อ section ไม่ใช่ปุ่มสีรุ้งเรียงกัน
```

---

## 12. UX Flow — "คลิกน้อย · ข้อมูลเชื่อมกัน" (LOCKED 2026-06-02)

> CEO 2026-06-02: "design โง่ ใช้งานจริงยาก กดหลายคลิกไปหลายหน้า ข้อมูลไม่เชื่อมกันทั้งที่ควรเชื่อม"

### 12.1 ลดจำนวนคลิก
```
✓ ถึง action สำคัญใน ≤ 2-3 คลิกจากหน้าแรกของโมดูล
✓ งานที่ทำบ่อย → ปุ่มลัดบน dashboard / nav ตรง ๆ
✓ แก้ไขเร็ว → inline-edit หรือ drawer/modal แทนเด้งไปหน้าใหม่
✗ บังคับเดิน 4-5 หน้าเพื่อทำงานเดียว · ฟอร์มหลายสเต็ปทั้งที่ใส่ครั้งเดียวจบได้
```

### 12.2 เชื่อมข้อมูลที่เกี่ยวกัน (linked records)
```
✓ ทุกการอ้างถึง entity อื่น = ลิงก์คลิกไปได้ (สาขา→หน้าสาขา · พนักงาน→โปรไฟล์ · บิล→ผู้ขาย)
✓ หน้า detail โชว์ "ของที่เกี่ยวข้อง" (สาขานี้มีบิล/ตู้/รายงานอะไรล่าสุด)
✓ breadcrumb บอกตำแหน่ง + กลับได้ใน 1 คลิก
✗ ผู้ใช้ต้องจำ ID แล้วไปค้นเองอีกหน้า · กรอกข้อมูลเดียวกันซ้ำหลายที่
```

### 12.3 ตรวจก่อนสร้าง/แก้ UI ทุกครั้ง (ผูกกับ CLAUDE.md RULE D#4)
```
☐ จากหน้านี้ ทำงานหลักเสร็จใน ≤ 3 คลิกไหม?
☐ ทุกชื่อ entity ที่โชว์ คลิกไปหน้าที่เกี่ยวข้องได้ไหม?
☐ มีข้อมูลที่ผู้ใช้ต้องไปหาเองจากหน้าอื่นไหม? → ดึงมาโชว์ตรงนี้
☐ ผ่าน §11 (ไม่มี emoji · ไอคอน lucide ชุดเดียว · ไม่เหมือน AI) ไหม?
```

---

*Updated 2026-06-02 — เพิ่ม §11 Anti-AI-Slop + §12 UX Flow (CEO: เลิกหน้าตาเหมือนฟรี AI · ลดคลิก · ลิงก์ข้อมูล)*
*Prev: 2026-05-05 Brand DNA*
*Memory: [[ceo-quality-bar-skill-playbook-2026-06-02]] · feedback_design_brand_dna.md*
