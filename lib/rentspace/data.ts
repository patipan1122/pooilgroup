...
export async function listContracts(orgId: string, projectId?: string) {
  return prisma.rentalContract.findMany({
    ...
    orderBy: { createdAt: "desc" },
    // select (not include) on purpose — list views (contracts, bills page's
    // active-contracts picker) never read signatureDataUrl / customTermsHtml /
    // buildingModifications / editProposedBodyHtml, but those are large
    // base64/HTML text columns that used to ride along on every row via
    // `include`. Every other RentalContract scalar column is kept as-is
    // (zero behavior change) — only those 4 heavy columns are dropped.
    select: {
      id: true,
      orgId: true,
      projectId: true,
      ...
      rentRequestedBy: true,
      rentRequestedAt: true,
      rentDecidedBy: true,
      rentDecidedAt: true,
      rentDecisionNote: true,
      pendingPromoDiscountThb: true,
      discountApprovalStatus: true,
      discountRequestedBy: true,
      discountRequestedAt: true,
      discountDecidedBy: true,
      discountDecidedAt: true,
      discountDecisionNote: true,
      note: true,
      createdBy: true,
      createdAt: true,
      updatedAt: true,
