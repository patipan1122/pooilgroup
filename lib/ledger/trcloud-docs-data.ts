...
export async function getTrcloudDocs(orgId: string, f: TrcloudDocFilters): Promise<TrcloudDocsData> {
  ...
  const [rows, countByKind, cf, dept, proj, status, lastSync] = await Promise.all([
    prisma.ledgerTrcloudDoc.findMany({
      ...
      take: ROW_LIMIT + 1,
      // select (not full row) — `raw` (Json?, "แถวดิบทั้งก้อน เผื่ออนาคต") is never
      // read by the mapping below or by the page; everything else that IS read
      // (see the `mapped` block further down) stays.
      select: {
        id: true,
        kind: true,
        trcloudId: true,
