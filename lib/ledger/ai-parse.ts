...

{
  ...
  "doc_date": "<วันที่ในเอกสาร รูปแบบ YYYY-MM-DD เป็น ค.ศ. เสมอ หรือ null · ถ้าบิลใช้ปี พ.ศ. (2400 ขึ้นไป เช่น 2568) ให้ลบ 543 เป็น ค.ศ. ก่อน (2568→2025)>",
  "subtotal": <ยอดรวมค่าสินค้า/บริการ ก่อนหักส่วนลดท้ายบิล และก่อน VAT = ผลรวม amount ทุกบรรทัด ปกติต้อง = Σ items.amount (ข้อยกเว้น: ถ้าท้ายบิลพิมพ์ยอด "ก่อนภาษี/Net" แยกไว้ชัดเจนแล้วไม่ตรงกับ Σ items.amount → ราคาต่อชิ้นในตารางน่าจะรวม VAT อยู่แล้ว ให้เชื่อยอด "ก่อนภาษี" ท้ายบิลแทน ดูกฎ VAT ข้อย่อยด้านล่าง) · ห้ามเอายอด "คงเหลือ/หลังหักส่วนลด" ที่อยู่เหนือบรรทัด VAT มาใส่ช่องนี้ — number ไม่มีคอมม่า หรือ null>,
...
- ตัวเลขทั้งหมดเป็น number ตรง ๆ ไม่มีคอมม่า ไม่มี ฿ · เลขไทย ๐๑๒๓๔๕๖๗๘๙ แปลงเป็นเลขอารบิก 0-9 ก่อนเสมอ (ทั้งยอดเงินและวันที่)
...
กฎชื่อผู้ขาย (vendor — อ่านให้ครบ ห้ามเหลือตัวอักษรเดียว):
- vendor = ชื่อ "ผู้ขาย/ร้านที่ออกบิล" เต็ม ๆ ที่พิมพ์ตัวใหญ่บน "หัวเอกสาร" บนสุด — มักขึ้นต้นด้วย "ห้างหุ้นส่วนจำกัด / บริษัท ... จำกัด / ร้าน ..." · อ่านชื่อให้ครบทั้งบรรทัด ห้ามตัดเหลือคำเดียวหรือตัวอักษรเดียว
- ห้ามเอาชื่อ "ผู้ซื้อ/ลูกค้า/ในนาม" มาเป็น vendor เด็ดขาด — โดยเฉพาะ "บริษัท เจพีซิ่งค์ กรุ๊ป จำกัด" (JP Sync / เจพีซิ่งค์) = ผู้ซื้อ (พวกเราเอง) ไม่ใช่ผู้ขาย
- ห้ามเอาชื่อ "พนักงานขาย/ผู้ติดต่อ/คุณ…" หรือโลโก้/ตัวย่อธนาคาร (K, SCB, KBANK) มาเป็น vendor — หา "ชื่อนิติบุคคลเต็ม" ที่พิมพ์บนหัวบิล
- อ่านชื่อผู้ขายไม่ออกจริง ๆ → คืน null (ห้ามเดาเป็นตัวอักษร/คำเดียว เช่น "K")
...
- ⚠️ เอกสารบางแบบ (เช่น ใบยืนยันคำสั่งซื้อ/ใบสั่งซื้อของร้านค้าปลีกวัสดุก่อสร้าง) พิมพ์ราคาต่อชิ้นรวม VAT ไว้ในตารางแล้ว (Σ items.amount จะ = ยอดรวมสุทธิพอดี) แต่ท้ายบิลมีบรรทัดสรุปแยกต่างหากชัดเจนเป็น 3 บรรทัดติดกัน คล้าย "มูลค่าสินค้า(และค่าขนส่ง)ก่อนภาษี / ก่อนVAT / Net" → "ภาษี / VAT 7%" → "รวมเงินสุทธิ" — กรณีนี้ **ให้เชื่อ 3 บรรทัดนี้ตรง ๆ เป็น subtotal/vat/total เสมอ แม้จะไม่เท่ากับ Σ items.amount** (ห้ามฝืนให้ vat=0 เพื่อให้ subtotal=Σitems — ราคาต่อชิ้นแค่รวม VAT ไว้แล้วเฉย ๆ ไม่ได้แปลว่าไม่มี VAT)
- วิธีแยกกรณีนี้จากกรณี "ราคารวม VAT ไม่มีบรรทัดแยก" (ข้อด้านบน): ต้องเห็นตัวเลข VAT ที่ไม่ใช่ 0 พิมพ์ไว้ชัดเจนเป็นบรรทัดของตัวเอง — ถ้าเห็นแบบนั้นคือกรณีนี้ ไม่ใช่กรณี vat=0
...
ตรวจก่อนตอบ (บังคับ): subtotal − discount + vat − wht ต้อง = total (คลาด ≤ 1 บาท) เสมอ · Σ items.amount = subtotal เฉพาะกรณีปกติ — ถ้าท้ายบิลพิมพ์บรรทัด "ก่อนภาษี/VAT/สุทธิ" แยกชัดเจน (ตัวอย่าง จ) ให้ Σ items.amount ≠ subtotal ได้ (ราคาต่อชิ้นรวม VAT ไว้แล้ว) โดยไม่ต้องแก้ · ถ้าสมการ subtotal−discount+vat−wht=total ไม่ตรงและไม่ใช่กรณีนี้ → ส่วนใหญ่เกิดจากแกะ VAT บนบิลราคารวม (แก้: vat=0) หรือหักส่วนลดซ้ำ หรือหยิบเงินสด/เงินทอนมา → ทบทวนแล้วอ่านใหม่ก่อนคืนค่า
...
(ง) ชื่อผู้ขาย: หัวบิลพิมพ์ "ห้างหุ้นส่วนจำกัด ขวัญชัย อิเล็คทริค แอนด์ ไลท์ติง" · ผู้ซื้อคือ "บริษัท เจพีซิ่งค์ กรุ๊ป จำกัด" → vendor ต้องเป็นชื่อผู้ขายเต็ม (ห้ามเป็น "K" หรือชื่อผู้ซื้อ)
{"vendor":"ห้างหุ้นส่วนจำกัด ขวัญชัย อิเล็คทริค แอนด์ ไลท์ติง","subtotal":1816,"discount":0,"vat":127.12,"wht":0,"total":1943.12}
(จ) ใบยืนยันคำสั่งซื้อร้านวัสดุก่อสร้าง: ตารางสินค้าราคารวม VAT ต่อชิ้น รวมได้ 894.00 (= Σ items.amount) แต่ท้ายบิลพิมพ์แยก 3 บรรทัดชัดเจน "มูลค่าสินค้าและค่าขนส่งก่อนภาษี(Net): 835.51" → "ภาษี(VAT 7%): 58.49" → "รวมเงินสุทธิ: 894.00" → ต้องเชื่อ 3 บรรทัดท้ายบิลนี้ ไม่ใช่ฝืนให้ vat=0 เพราะ Σ items.amount เท่ากับ total พอดี:
{"subtotal":835.51,"discount":0,"vat":58.49,"wht":0,"total":894.00,"items":[{"description":"กาว PVC 500 กรัม","qty":1,"unit_price":272,"amount":272,"vat_rate":0.07},{"description":"ฝาครอบ PVC 13.5\\"","qty":3,"unit_price":10,"amount":30,"vat_rate":0.07}]}`;
...
// ชื่อผู้ขายที่ "สั้นผิดปกติ" — ตัดช่องว่าง/จุด/ขีดออกแล้วเหลือ ≤ 2 ตัว (เช่น "K", "AB") =
// AI อ่านชื่อร้านพลาดแน่ ๆ (ชื่อนิติบุคคลไทยยาวกว่านั้นเสมอ). ใช้ลด confidence กัน false 100%.
function isSuspiciousVendorName(v: string | null): boolean {
  if (!v) return false;
...
export async function parseReceipt(
  ...
  // ชื่อผู้ขายสั้นผิดปกติ (เช่น "K" ตัวเดียว) = AI อ่านพลาด (รูปเอียง/พื้นหลังรก จับชื่อได้แค่เศษ) →
  // อย่าปล่อยให้โชว์ "มั่นใจ 100%" บนชื่อผิด · กดมั่นใจต่ำ ให้คนตรวจ/แก้ (บิลตั้ง needs_review อยู่แล้ว).
  if (isSuspiciousVendorName(result.vendor)) {
    result.confidence.vendor = Math.min(result.confidence.vendor ?? 0, 0.2);
  }
