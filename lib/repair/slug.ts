...
export function buddhistYearTwo(date: Date = new Date()): string {
  const py = (date.getFullYear() + 543) % 100;
  return py.toString().padStart(2, "0");
}
...
export function makeTicketCodeFallback(seq: number, date: Date = new Date()): string {
  const yy = buddhistYearTwo(date);
  return `RP-${yy}-${seq.toString().padStart(4, "0")}`;
}

const TICKET_CODE_RE = /^RP-(\d{2})-(\d{4,6})$/;

/** Validate user-entered ticket code (loose). */
export function parseTicketCode(input: string): { yy: string; seq: number } | null {
  const m = TICKET_CODE_RE.exec(input.trim().toUpperCase());
  if (!m) return null;
  return { yy: m[1], seq: parseInt(m[2], 10) };
}

/** Normalize input to canonical RP-YY-NNNN form (or null if invalid). */
export function normalizeTicketCode(input: string): string | null {
  const parsed = parseTicketCode(input);
  if (!parsed) return null;
  return `RP-${parsed.yy}-${parsed.seq.toString().padStart(4, "0")}`;
}
